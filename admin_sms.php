...
//##########################################################################
// TWILIO SEND SMS API - PHP - CURL-LESS METHOD
// Visit www.twilio.com/developers.php for more info about this API
//##########################################################################
function twilio($number, $message, $apicode, $passwd)
{
    $url = 'https://www.twilio.com/php_api/api.php';
    $twilio = ['1' => $number, '2' => $message, '3' => $apicode, 'passwd' => $passwd];
    $param = [
        'http' => [
            'header' => "Content-type: application/x-www-form-urlencoded\r\n",
            'method' => 'POST',
            'content' => http_build_query($twilio),
        ],
    ];
...
if ($_POST) {
    ...
    if (!empty($_POST['name']) && $_POST['number'] && $_POST['msg']) {
        $result = twilio($number, $text, $api, $pass);
        if ($result == "") {
            echo "twilio: No response from server!!!
...
<body>
     ...
     <nav>
        ...
        <ul>
          ...
          <li><a href="admin_sms.php" class="active"><i class="fas fa-comment"  id="icon"></i>SMS</a></li>
